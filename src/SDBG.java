...
public class SDBG extends Programmer {
  ...
  // Keys
  public static final byte[]  ChipErase = {0x65, 0x73, 0x61, 0x72, 0x45, 0x4D, 0x56, 0x4E};   // 0x4E564D4572617365
  public static final byte[]  NvmProg   = {0x20, 0x67, 0x6F, 0x72, 0x50, 0x4D, 0x56, 0x4E};   // 0x4E564D50726F6720
  public static final byte[]  UserRow   = {0x65, 0x74, 0x26, 0x73, 0x55, 0x4D, 0x56, 0x4E};   // 0x4E564D5573267465
  public static final byte[]  Attach    = {0x20, 0x20, 0x20, 0x20, 0x20, 0x44, 0x43, 0x4F};   // 0x4F43442020202020
  // UPDI Registers
  ...
  void init () throws SerialPortException {
    ...
    do {
      try {
        ...
        stcs(UPDI_CTRLB, 1 << 3);                                     // disables response signatures generated by the UPDI
        stcs(UPDI_CTRLB, 1 << 7);                                     // enables fixed inter-byte delay between data bytes
        ldcs(UPDI_STATUSA);
        ldcs(UPDI_STATUSA);
  ...
  public byte[] getBytes (int size) throws SerialPortException {
    try {
      return jPort.readBytes(size, 500);
    } catch (SerialPortTimeoutException ex) {
      throw new IllegalStateException("SDBG.readBytes() Timeout");
    }
  }
